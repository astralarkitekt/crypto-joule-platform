...
export const useCryptoJouleStore = defineStore({
  ...
  actions: {
    async initializeStore(blockHeight) {
      try {
        ...
        if (!blockHeight > bestBlockHeight) {
          console.error(
            "Block height exceeds best block height",
            blockHeight,
            bestBlockHeight
          );
          return;
    ...
    async getParcelData(parcelIndex) {
      try {
        const txnHash = this.blockTransactions[parcelIndex];
        if (!txnHash) {
          return null;
        }

        const txn = await axios.get(`${API_URL}transaction/${txnHash}`);
        return txn.data;

      } catch (e) {
        console.error("Error getting parcel data: ", e);
      }
    },
  },
});
